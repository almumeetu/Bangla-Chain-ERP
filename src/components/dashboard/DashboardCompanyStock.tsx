    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      <div className="bg-white rounded-none border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2.5 py-0.5 rounded-none border border-slate-200">
        ...
        <div className="p-4 space-y-2 max-h-[340px] overflow-y-auto">
          {companyStockData.map((comp, i) => {
            ...
            return (
              <div key={comp.brand} className="flex items-center gap-3 p-3 rounded-none border border-slate-100 bg-slate-50/30 hover:bg-white hover:border-slate-200 transition-all">
                <div className={`w-9 h-9 rounded-none ${c.bg} flex items-center justify-center text-white font-bold text-[11px] shrink-0 shadow-sm`}>
                  {comp.brand[0]}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-slate-200/60 rounded-none overflow-hidden">
                      <div className={`h-full ${c.bg} rounded-none transition-all duration-700`} style={{ width: `${Math.max(6, pct)}%` }} />
                    </div>
      ...
      <div className="bg-white rounded-none border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2.5 py-0.5 rounded-none border border-amber-100">
            {bn ? 'ড্যামেজ' : 'Damage'}
          </span>
        </div>
        <div className="p-4 space-y-2 max-h-[340px] overflow-y-auto">
          {companyStockData.map(comp => {
            ...
            return (
              <div key={comp.brand} className="flex items-center gap-3 p-3 rounded-none border border-slate-100 bg-slate-50/30 hover:bg-white hover:border-slate-200 transition-all">
                <div className="w-9 h-9 rounded-none bg-amber-100 flex items-center justify-center text-amber-700 font-bold text-[11px] shrink-0">
                  {comp.brand[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-slate-200/60 rounded-none overflow-hidden">
                      <div className="h-full bg-rose-400 rounded-none transition-all duration-700" style={{ width: `${Math.max(pct > 0 ? 6 : 0, pct)}%` }} />
                    </div>
                    <span className="text-[9px] font-semibold text-slate-400 shrink-0 w-14 text-right">
